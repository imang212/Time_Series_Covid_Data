...
start_week <- min(data$month[data$year == start_year])
ts_daily_data <- ts(data$new_cases, start = c(start_year, start_month), frequency = 52)

# Klouzavé průměry různých řádů
ma_3 <- SMA(data$new_cases, n = 4) 
ma_12 <- SMA(data$new_cases, n = 52) 

# Exponenciální vyrovnání - jednodušší přístup
exp_smooth_model <- HoltWinters(ts_daily_data, gamma = FALSE)

# Vytvoříme jednoduché exponenciální vyhlazení pomocí ETS
ets_model <- ets(ts_daily_data, model = "AAN", damped = FALSE)
...
trend_plot <- trend_data %>%
  ...
  scale_color_manual(values = c("original" = "gray50", "ma_3" = "green", "ma_12" = "red", "exp_smooth" = "purple"),
                     name = "Metoda",
                     labels = c("original" = "Originální data", 
                                "ma_3" = "MA(3)", 
                                "ma_12" = "MA(12)", "exp_smooth" = "Exp. vyhlazení")) +
...
data$sin_1 <- sin(2 * pi * data$t / 1) * 0.1
data$cos_1 <- cos(2 * pi * data$t / 1) * 0.1
