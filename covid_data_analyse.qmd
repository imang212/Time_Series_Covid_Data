...
Stabilita: Trend v našem případě není stabilní; vykazuje několik výrazných vln, které odpovídají jednotlivým fázím pandemie. Metoda MA(52) nejlépe vyhlazuje sezónní šum a odhaluje dlouhodobý klesající směr v závěru sledovaného období.s
...
Vlny COVID-19 byly v tomto období více ovlivněny příchodem nových variant nebo vládními opatřeními než fixním kalendářním ročním cyklem, který tyto goniometrické funkce popisují. Rovnice modelu: $$\widehat{\ln}(1 + \text{new\_cases}_t) = 15,18 - 7,71 \cdot \text{poly}(t, 2)_1 - 27,64 \cdot \text{poly}(t, 2)_2 - 0,06 \cdot \sin\left(\frac{2\pi t}{365}\right) - 0,11 \cdot \cos\left(\frac{2\pi t}{365}\right)$$\
...
$$W_t = 0,0981 W_{t-1} + 0,2600 W_{t-2} - 0,1673 W_{t-3} - 0,2956 W_{t-4} + 0,0421 W_{t-5} + \epsilon_t - 0,8147 \epsilon_{t-1}$$ Model ARIMA(5,2,1) využívá 5 autoregresních členů a dvojitou diferenciaci k odstranění nestacionarity (trendu)
...
Kontrola ukazuje, zda jsou splněny základní předpoklady modelů - nezávislost residuí (ACF, Ljung-Box test): - SARIMA a ARIMAX se prokázala nezávislost reziduí p \> 0.0. Rezidua jsou nezávislá a modely správně zachytily časovou dynamiku pandemie. - Funkční model - p = 0 znamená, že model je špatný. V reziduích zůstala silná autokorelace, kterou model neumí vysvětlit. Při autokorelaci: Bylo by nutné zvýšit řád ARIMA modelu (parametry $p$ nebo $q$), abychom zachytili zbývající závislost.

Normalita (Shapiro test, Q-Q ploty se norprokázala ani u jednoho).
