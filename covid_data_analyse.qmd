...
ADF a KPSS testy říkají, že řada je nyní stacionární (p\<=0.05).
...
# Vlastní model
(fit_own <- Arima(ts_log, order = c(1,0,2), seasonal = list(order = c(0,0,0))))
...
external_vars <- cbind(stringency_lag = data$stringency_lag, hosp_lag = data$hosp_lag)
...
model_bc <- lm(bc_new_cases ~ poly(t, 2), data = data)
summary(model_bc)
...
pred_func <- predict(model_no_season, newdata = future_data, interval = "prediction")
# zpět na původní škálu
pred_func <- expm1(pred_func)
